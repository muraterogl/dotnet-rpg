    ...
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<AttackResultDto>> WeaponAttack(WeaponAttackDto request)
        {
            ...
            try
            {
                ...
                Character oppenent = await _context.Characters
                    .FirstOrDefaultAsync(c => c.Id == request.OpponentId);
                int damage = DoWeaponAttack(attacker, oppenent);
                if (oppenent.HitPoints <= 0)
                {
                    response.Message = $"{oppenent.Name} has been defeated!";
                }
                _context.Characters.Update(oppenent);
                await _context.SaveChangesAsync();
                response.Data = new AttackResultDto
                {
                    Attacker = attacker.Name,
                    AttackerHP = attacker.HitPoints,
                    Opponent = oppenent.Name,
                    OpponentHP = oppenent.HitPoints,
                    Damage = damage
                };
            }
            catch (Exception ex)
        ...
        private static int DoWeaponAttack(Character attacker, Character oppenent)
        {
            ...
            damage -= new Random().Next(oppenent.Defense);
            if (damage > 0)
            {
        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack(SkillAttackDto request)
        {
            ...
            try
            {
                ...
                int damage = DoSkillAttack(attacker, oppenent, characterSkill);
                if (oppenent.HitPoints <= 0)
                {
                    response.Message = $"{oppenent.Name} has been defeated!";
                }
        ...
        private static int DoSkillAttack(Character attacker, Character oppenent, CharacterSkill characterSkill)
        {
